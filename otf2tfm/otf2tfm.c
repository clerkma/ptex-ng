...
static FT_Error err;I wd[256];S ht[256],dp[256];
FN(met){UI n;
  M($(FT_Load_Glyph(f,n,FT_LOAD_NO_SCALE),E("ld gyp %d failed\n",n),);
    wd[n]=g->advance.x;FT_Set_Pixel_Sizes(f,10,0);FT_Load_Glyph(f,n,0);
    FT_Render_Glyph(g,FT_RENDER_MODE_LIGHT);ht[n]=g->bitmap_top;dp[n]=g->bitmap.rows-ht[n];);}
