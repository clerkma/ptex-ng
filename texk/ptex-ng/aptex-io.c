...
// TODO: ".." and "."
char * aptex_utils_canonicalize (char * t)
{
  char * s = t;
...
/* patch path if (abstract -> concrete)
    (i)   path not empty
...
static inline void set_open_file (kpse_file_format_type file_fmt)
{
  switch (file_fmt)
  {
    case kpse_tex_format:
      open_file = aptex_env.open_tex_file;
      break;

    case kpse_tfm_format:
      open_file = aptex_env.open_tfm_file;
      break;

    case kpse_fmt_format:
      open_file = aptex_env.open_fmt_file;
      break;
  }
}
...
static char * xstrdup_name (void)
{
  ...
  {
    (void) getcwd(aptex_log_line, sizeof(aptex_log_line));
    strcat(aptex_log_line, DIR_SEP_STRING);
  }

  strcat(aptex_log_line, (char *) name_of_file + 1);
  aptex_utils_canonicalize(aptex_log_line);

  return xstrdup(aptex_log_line);
}
  ...
  if (*f == NULL)
  {
    ...
    if (temp_dir != NULL)
    {
      ...
      if (aptex_env.flag_deslash)
        aptex_utils_canonicalize((char *) temp_name);
