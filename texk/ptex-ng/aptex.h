...
#define generic_dump(x)           block_dump  ((char *) &(x), sizeof (x), 1, fmt_file) //dump_things(x, 1)
#define generic_undump(x)         block_undump((char *) &(x), sizeof (x), 1, fmt_file) //undump_things(x, 1)
...
EXTERN struct {
  ...
  boolean trace_csname;
  boolean trace_caret;
  boolean trace_tfm;
  boolean trace_linebreak;
  // flags
  boolean flag_initex;
  boolean flag_verbose;
  boolean flag_tex82;
  boolean flag_compact_fmt;
  boolean flag_c_style;
  boolean flag_deslash;
  boolean flag_caret_dec;
  boolean flag_caret_hex;
  boolean flag_civilize_date;
...
EXTERN char aptex_log_line[256];
