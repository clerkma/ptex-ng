...
c
c 3.00
c  Provides an option to remove the limit of 9 on forced
c  beams. A side benefit of selecting the option is that all
c  beams are vectorized and 2-digit figured bass figures are allowed.
c 2.98
...
c
     ...
     *                fullsize(nm),ivxfig2,ivvfig(2,74)
      ...
      common /comslope/ bigslope,novbm
...
c          if (abs(islope(ivx)) .gt. 9) islope(ivx) = sign(9,islope(ivx))
          if (abs(islope(ivx)).gt.9.and..not.bigslope)
     *                     islope(ivx) = sign(9,islope(ivx))
...
c
      ...
      if (islope(ivx) .lt. -9) then
       ...
       write(notexq(lnote-3:lnote),'(i3,a1)')islope(ivx),'}'
      else if (islope(ivx) .lt. 0) then
        notexq = notexq(1:lnote)//noteq(1:lnoten)//'{'
        lnote = lnote+4+lnoten
        write(notexq(lnote-2:lnote),'(i2,a1)')islope(ivx),'}'
      else if (islope(ivx) .le. 9) then 
        notexq = notexq(1:lnote)//noteq(1:lnoten)
...
c
          call g1etchar(lineq,iccount,durq)
          if (durq .eq. chax(34)) then
c
c  Quoted name, go to next quote mark
c
            do 35 iccount = iccount+1 , 127
              if (lineq(iccount:iccount).eq.chax(34) .and. 
     *            lineq(iccount-1:iccount-1).ne.chax(92)) go to 36
35          continue
            call errmsg(lineq,iccount,ibarcnt-ibaroff+nbars+1,
...
c
     ...
     *        '"AN" must be followed by inst. #, then quote!')
          ...
          ndxquote = index(lineq(ndxquote+1:128),chax(34))
...
c
     ...
     *       'A number for vert shift after \eject must start here!')
          ...
          if (lineq(iccount+1:iccount+1).eq.'f') then
            iccount = iccount+1
            go to 27
          end if
          go to 27
          ...
          call errmsg(lineq,iccount,ibarcnt-ibaroff+nbars+1,
c     *       'After "A" must follow one of the letters abBcdeiINprRsST!')
...
c
        ...
        iccountt = iccount
3       continue
        iend = iccountt+index(lineq(iccountt+2:128),chax(34))+2
        if (lineq(iend-2:iend-2) .eq. chax(92)) then
          iccountt = iend-2
          go to 3
        end if
...
c
            ...
            do 35 iccount = namstrt+1, 128
              if ((quoted .and. lineq(iccount:iccount).eq.chax(34) .and.
     *             lineq(iccount-1:iccount-1).ne.chax(92)) .or.
...
c
     ...
     *    inameq,clefq,sepsymq,pathnameq,lpath,isig0)
      parameter (nm=24)
      common /comnvi/ nsperi(nm),nspern(nm),rename,iiorig(nm)
      character*1 clefq(nm),sepsymq(nm),chax
      character*40 pathnameq
...
12    ipos = index(pathnameq,chax(92))
      if (ipos .gt. 0) then
        pathnameq(ipos:ipos)=chax(47)
        go to 12
      end if
      return
      end
...
c
      ...
      igetvarlen = 0
      do 1 nbytes = 1 , 4
        mmidi4 = mmidi(icm,imidi+nbytes)
        igetvarlen = 128*igetvarlen
     *               +iand(127,mmidi4)
        if (.not.btest(mmidi4,7)) return
1     continue
...
c Removed next line 211207 to fix beaming problem at new movement+meter change.
c              mtrnuml = 0
c
...
c
     ...
     *           8+lnof,soutq,lsout)
c
c  Mods for 2-digit fig with accidental.
c
            if (ch2q.eq.'1') then
              ic = ic+1
              lsout = lsout-1
              call addstr(figq(ic:ic)//'}',2,soutq,lsout)
            end if
          endif
...
c
          continue
        else if (ch1q .eq. '1') then
c
c  Mods for 2-digit fig with no accidental.
c
          ic = ic+1
          ch2q = figq(ic:ic)
          call addstr(sq//'off{-2pt}'//sq//Figutq//nofq(1:lnof)//'{'
     *           //'1'//ch2q//'}'//sq//'off{2pt}',28+lnof,soutq,lsout)
        else
c
...
c
      ...
      character*128 lineq
      character*1 durq,chax
      if (index(lineq(iccount+1:iccount+3),chax(47)) .eq. 0) then
c
...
c
        iccount = iccount+1
        ns = index(lineq(iccount:128),chax(47))
...
c
        ns = index(lineq(iccount:iccount+2),chax(47))
...
c
     ...
     *        inameq(iinsiv(iv))(1:lnam(iinsiv(iv)))
          if (debugmidi) write(52,'(a)')
     *        chax(34)//inameq(iinsiv(iv))(1:lnam(iinsiv(iv)))//chax(34)
        end if
        write(tempoq,'(i2)')icm
...
5     continue
c      write(*,'(1x,a12,(10i6))')'Bytes used:',(imidi(icm),icm=0,numchan)
c      write(15,'(1x,a12,(10i6))')
c     *                          'Bytes used:',(imidi(icm),icm=0,numchan)
      close(51)
...
c
      i2nd = iccount+index(lineq(iccount+1:128),chax(34))
2     continue
      if (lineq(i2nd-1:i2nd-1).eq.char(92)) then
        i2nd = i2nd+index(lineq(i2nd+1:128),chax(34))
        go to 2
      end if
      istart = iccount
