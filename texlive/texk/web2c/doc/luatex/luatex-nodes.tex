...
\subsection{\type {protect_glyph[s]} and \type {unprotect_glyph[s]}}
...
We assume subtypes to be in the range 0 (unset) upto 255 so in the \type {0x00FF}
bits of the (quarterword) subtype. Protected glyph have the \type {0xFF00} bits
set, which means that a subtype is now larger than 255. Unprotecting resets those
bits. The \type {node.is_char} helper uses the \type {0xFF00} mask to determine
if we have a processed character or not. In version 1.24 we changed to this
masking in favor of adding and subtracting where 256 was treated special, for
instance because we have a somewhat vague zero/one subtype situation and we also
set protection in math without an explicit subtype. This change is likely
harmless.
