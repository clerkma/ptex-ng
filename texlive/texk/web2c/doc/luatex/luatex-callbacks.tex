...
This is a \PDFTEX\ (?) compatibility features (put in the \ETEX\ extensions
namespace).

% In the perspective of \LUATEX\ it could have been a bitset because it somehow
% relates to splitting but the name of this callback and the related primitive has
% no relation to splitting. So be it.
...
This callback replaces the code that prints the error message when \prm
{ignoreprimitiveerror} is enabled. The usual interaction afterwards (one can
decide not to show a message) is not affected.
...
first operator is either \type {d0} or \type {d1}) and the width of the glyph (in
sp's) in mode~2. You can generally ignore mode~1. No sanity checks are done, so
whether it all works out well is up to the user.

\stopsection

\subsection{\cbk {dvi_output}}

\topicindex{callbacks+fonts}

The \type {dvi_output} callback can be triggered when the \DVI\ backend writes to
the \DVI\ file. Currently we only have one such location: when a character is
written to a file and the font has not been seen before.

\startfunctioncall
function(<number> where, <number> id)
    if where == 1 then
        return "using font id " .. id
    end
end
\stopfunctioncall

Of course, whatever gets written should be acceptable for the \DVI\ post
processor; no checking is done by \LUATEX. Future variants might pass different
arguments, depending on additional values of \type {where}.
