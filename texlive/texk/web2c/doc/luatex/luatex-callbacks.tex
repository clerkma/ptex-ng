...
The table is an array of known callback names. This can be useful for debugging,
such as when an error message displays the callback index instead of its name.
...
\subsection{\cbk {font_definition}}

\topicindex{callbacks+fonts}

\startfunctioncall
font_definition = function(<number> font_id)
   return <node> special | nil
end
\stopfunctioncall

The \type {font_id} is the internal number assigned to the font.

This callback is used in the \DVI\ backend.
If the special subtype whatsit node is returned,
writes it directly before the font definition command to the \DVI\ file.
It’s used by the \LaTeX\ package xdvipsk--support to provide
maplines for Open Type fonts,
similar to those in psfonts map files for Type1 fonts.
