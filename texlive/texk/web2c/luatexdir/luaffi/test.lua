...
local tp = ffi.metatype("struct newtest_1", {__new =
  function(tp, x, y, z)
...
local tp = ffi.metatype("struct newtest_2",
...
-- Test ffi.metatype
-- Ref: https://github.com/openresty/luajit2-test-suite/blob/master/test/ffi/ffi_metatype.lua
local ffi = require('ffi');
ffi.cdef[[
...
do
  ...
  --[[
    local f = 3LL / a
    3LL is part of C syntax, perhaps supported in LuaJit
	Adding support in luaffifb will be a huge task
  ]]
  local f = ffi.new("long long", 3) / a
...
do
  ...
  --[[
  __index member supports only functions or tables
  local xt = ffi.metatype("struct { }", { __index = fb })
  ]]
  local xt = ffi.metatype("struct { }", { __index = function (c, n) return  fb.x end })
  local o = xt()
  assert(o.x == 99)
end
