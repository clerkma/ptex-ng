...
static char glyph_name_buf[GLYPH_NAME_LEN + 1] = "";
static char *glyph_name_ptr = glyph_name_buf;
static int glyph_name_len = 0;
/* overridden getc() from the encoding vector file taking into account pending glyph name chars */
/* impossible to do it through ungetc() -- only single ungetc is guaranteed */
static int getc_glname(FILE *enc_file)
{
    int ch;
...
static void rename_enc_glname(FILE *enc_file, const char *pfb_name)
{
    char *glyph_subst;
    char *gl_pt, *dest_pt;
    int ch;
    ...
    *dest_pt++ = '\0';
    ...
    if (glyph_subst)
    {
        strncpy(glyph_name_buf, glyph_subst, GLYPH_NAME_LEN);
...
#ifdef XDVIPSK
      {
          resfont_ref *p_ref = NULL;
          char *font_file = NULL;
          const char *pfb_fname = NULL;
...
drawchar(chardesctype *c, int cc)
{
   ...
   if (lua_after_drawchar) {
      ...
      if (curfnt->resfont->otftype) {
         luamaptype *map, *current = NULL;
         int icid;
         map = LuaMap_cache_get(curfnt->resfont->luamap_idx);
         icid = c->cid;
         HASH_FIND_INT(map, &icid, current);
         if (current != NULL) {
            tu_count = current->tu_count;
            tounicode = current->tounicode;
            run_lua_after_drawchar(L, c, cc, rhh, rvv, dir, lastfont, tu_count, tounicode);
         } else {
            run_lua_after_drawchar(L, c, cc, rhh, rvv, dir, lastfont, 0, 0);
         }
      } else {
         if (curfnt->resfont->enc && (c->charcode >= 0) && (c->charcode < 256)) {
             char *font_file = NULL;
             char *pfb_name = NULL;
             if (((font_file = curfnt->resfont->Fontfile) != NULL) && strstr(font_file, PFB_EXT))
                 pfb_name = extract_fname(font_file, NULL);
             else
                 pfb_name = strdup("");
             assert(pfb_name);
             tounicode = get_glname_tounicode(curfnt->resfont->enc[c->charcode], pfb_name, "pfb:", FALSE, &tu_count, NULL);
             free(pfb_name);
         }
         if (tounicode) {
            run_lua_after_drawchar(L, c, cc, rhh, rvv, dir, lastfont, tu_count, tounicode);
