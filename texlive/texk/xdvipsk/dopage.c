...
#else
   cd = NULL;
   if (mychar<curfnt->maxchars || (!noluatex && curfnt->resfont && curfnt->resfont->otftype)) {
...
#ifdef XDVIPSK
         if (mychar < curfnt->localfonts->desc->maxchars && cd && (cd0[mychar].flags2 & EXISTS)) {
#else
