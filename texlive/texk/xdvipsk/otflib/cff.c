...
cff_close (cff_font *cff)
{
  ...
  if (cff) {
    ...
    if (cff->privat) {
      for (i=0;i<cff->num_fds;i++) {
	if (cff->privat[i]) cff_release_dict(cff->privat[i]);
      }
      RELEASE(cff->privat);
    }
    if (cff->subrs) {
...
long cff_read_subrs (cff_font *cff)
{
  ...
  if (cff->flag & FONTTYPE_CIDFONT) {
    for (i=0;i<cff->num_fds;i++) {
      if (cff->privat[i] == NULL ||
	  !cff_dict_known(cff->privat[i], "Subrs")) {
	(cff->subrs)[i] = NULL;
      } else {
	offset = (long) cff_dict_get(cff->fdarray[i], "Private", 1);
	offset += (long) cff_dict_get(cff->privat[i], "Subrs", 0);
	cff_seek_set(cff, offset);
    ...
    if (cff->privat[0] == NULL ||
	!cff_dict_known(cff->privat[0], "Subrs")) {
      (cff->subrs)[0] = NULL;
    } else {
      offset = (long) cff_dict_get(cff->topdict, "Private", 1);
      offset += (long) cff_dict_get(cff->privat[0], "Subrs", 0);
      cff_seek_set(cff, offset);
...
long cff_read_private (cff_font *cff)
{
  ...
  if (cff->flag & FONTTYPE_CIDFONT) {
    ...
    cff->privat = NEW(cff->num_fds, cff_dict *);
    for (i=0;i<cff->num_fds;i++) {
	...
	(cff->privat)[i] = cff_dict_unpack(data, data+size);
	RELEASE(data);
	len += size;
      } else {
	(cff->privat)[i] = NULL;
	...
	(size = (long) cff_dict_get(cff->topdict, "Private", 0)) > 0) {
      ...
      if (cff_read_data(data, size, cff) != size)
	ERROR("reading file failed");
      cff->privat[0] = cff_dict_unpack(data, data+size);
      RELEASE(data);
      len += size;
    } else {
      (cff->privat)[0] = NULL;
