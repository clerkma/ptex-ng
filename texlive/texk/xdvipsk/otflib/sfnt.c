...
static long stm_size;

static void add_stream(BYTE **stm, BYTE *src, long size)
{
	if ( *stm == NULL ) {
		*stm = NEW(size,BYTE);
		stm_size = 0;
		...
		*stm = RENEW(*stm,stm_size + size,BYTE);
	}
	memcpy(*stm + stm_size,src,size);
	stm_size += size;
}

long
sfnt_make_fontfile (sfnt *sfont, BYTE **buffer)
{
  BYTE *stream;
  struct sfnt_table_directory *td;
  ...
  for (i = 0; i < td->num_tables; i++) {
    if (td->flags[i] & SFNT_TABLE_REQUIRED) {
      if ((offset % 4) != 0) {
	length  = 4 - (offset % 4);
	add_stream(&stream, padbytes, length);
	offset += length;
      }
      if (!td->tables[i].data) {
	if (!sfont->stream)
	...
	while (length > 0) {
	  ...
	  if (nb_read < 0) {
	    ...
	    add_stream(&stream, wbuf, nb_read);
	  }
	  length -= nb_read;
	}
      } else {
