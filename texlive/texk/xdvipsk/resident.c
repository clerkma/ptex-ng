...
#ifdef XDVIPSK
resfont_ref *reshash_ps[RESHASHPRIME];
resfont_ref *reshash_v[RESHASHPRIME];
#endif /* XDVIPSK */
...
#ifdef XDVIPSK
boolean otftype_conforms(quarterword /* otftype_enum */ font_otftype, quarterword /* otftype_enum */ otftype)
{
    boolean retv = false;
    ...
    {
    case Any_type1_font:
        if ((font_otftype == None_font) /* ||
            (font_otftype == PostScript_font) */)
            retv = true;
        break;
    case Any_otf_font:
        if ((font_otftype != None_font) /* &&
...
resfont_ref *lookup_ps(const char *ps_name, quarterword /* otftype_enum */ otftype)
{
    resfont_ref *ptr;

    assert(ps_name);

    for (ptr = reshash_ps[hash(ps_name)]; ptr != NULL; ptr = ptr->next)
        if ((strcmp(ptr->resfont_ptr->PSname, ps_name) == 0) && otftype_conforms(ptr->resfont_ptr->otftype, otftype))
            break;

    return(ptr);
}

resfont_ref *lookup_v(const char *vect_name)
{
  resfont_ref *ptr;

  assert(vect_name);

  for (ptr = reshash_v[hash(vect_name)]; ptr != NULL; ptr = ptr->next)
    if (strcmp(ptr->resfont_ptr->Vectfile, vect_name) == 0)
      break;
...
struct resfont *
#ifdef XDVIPSK
findPSname(char *name, quarterword /* otftype_enum */ otftype)
#else
...
#ifdef XDVIPSK
            if (otftype_conforms(p->otftype, otftype))
#endif /* XDVIPSK */
            return p;
      }
   return NULL;
...
#endif /* XDVIPSK */
{
   ...
   reshash[h] = p;
#ifdef XDVIPSK
   p_ref = lookup_ps(PSname, p->otftype);
   if (p_ref == NULL)
   {
      p_ref = (resfont_ref*)mymalloc((integer)sizeof(resfont_ref));
      assert(p_ref);

      h = hash(PSname);
      p_ref->next = reshash_ps[h];
      reshash_ps[h] = p_ref;
   }
   p_ref->resfont_ptr = p;
...
#else /* XDVIPSK */
#ifdef KPATHSEA
               SET_CLIENT_PATH (kpse_miscfonts_format, fliparse(PSname,tempname));
#else
               flipath = getpath(fliparse(PSname,tempname), flipath);
#endif
#endif
...
void parse_otf_map_line(const char *map_line)
{
   char *map_line_copy = NULL;
   register char *p, *pt;
   char* specinfo;
   char specbuf[500];
   char map_name[260];

   p = map_line_copy = strdup(map_line);
   assert(map_line_copy);
   ...
   if (*p > '\t' && *p != '*' && *p != '#' && *p != ';' && *p != '%') {
      ...
      while (*p) {
         ...
         if (*p) {
            ...
            if (hdr_name) {
               ...
               pt = hdr_name + strlen(hdr_name) - 1;
               while ((pt >= hdr_name) && ((*pt == '\n') || (*pt == '\r')))
                   *pt-- = '\0';
      ...
      if (TeXname) {
         ...
         char *token;
         float val;
         pt = mymalloc(strlen(TeXname) + 1);
         strcpy(pt, TeXname);
         strcat(specbuf, "{");
         token = strtok(pt, seps);
         while( token != NULL ) {
