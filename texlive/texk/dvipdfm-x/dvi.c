...
int
dvi_locate_expanded_font (const char *tfm_name, const char *tex_name,
                          spt_t ptsize, int extend)
{
  int           cur_id = -1;
  fontmap_rec  *mrec;
  const char   *name = tex_name;
  int           subfont_id = -1, font_id; /* VF or device font ID */
  ...
  font_id = pdf_dev_locate_expanded_font(name, tex_name, ptsize, 1 + ((double)extend / 1000.0));
