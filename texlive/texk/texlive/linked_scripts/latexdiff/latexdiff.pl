...
#    preamble file (via --preamble option). If the user-included preamble file does not define important commands, then they continue to be added (some simple
...
This is LATEXDIFF 1.4.0 (Algorithm::Diff $Algorithm::Diff::VERSION, Perl $^V)
...
my (
  $ARRENV,           $COUNTERCMD,  $FLOATENV,        $ITEMCMD,          $LISTENV,       $MATHARRENV,
  $MATHARRREPL,      $MATHENV,     $MATHREPL,        $MAXCHANGESLETTER, $MINWORDSBLOCK, $PICTUREENV,
  $SCALEDELGRAPHICS, $VERBATIMENV, $VERBATIMLINEENV, $CUSTOMDIFCMD,
);
...
# circumstances the ignoring of white space differences in conjunction with merging according to MINWORDSBLOCK rule
...
# standard $cmdoptseq (default: no intrevening spaces, controlled by extraspace) - a final open parentheses is merged to the command if it exists to deal properly with multi-argument text command
my $coords = '[\-.,\s\d]*';
#<<<  format skipping: do not let perltidy change my formatting
...
sub flatten {
  ...
  $text =~
    s/\\DIF((?:$VERBATIMENV|$VERBATIMLINEENV)(?:\[$brat_n\])?)\{([-\d]*?)\}/"\\begin{${1}}".fromhash(\%verbhash,$2)."\\end{${1}}"/esg;
...
##		      print STDERR "DEBUG Delblock Match math $& at ",pos,"\n";
    ...
    while (m/\\DIFdelbegin \\DIFdel\{($word\s*)\}\\DIFdelend \\DIFaddbegin \\DIFadd\{(${word}\s*)\}\\DIFaddend/g) {
      ...
      my $word1 = $1;
      my $word2 = $2;
      my @oldletters  = ( $word1 =~ /($word_cj |[-\w\d*]|\\[\"\'\`~^][A-Za-z\*])/g );
      my @newletters  = ( $word2 =~ /($word_cj |[-\w\d*]|\\[\"\'\`~^][A-Za-z\*])/g );
      
      ###print STDERR "DEBUG: Word detection. Found \\DIFdel{$word1}\\DIFadd{$word2} at ",pos,". Split into oldletters: |".join("|",@oldletters)."| and newletters: |".join("|",@newletters)."|\n" ;
